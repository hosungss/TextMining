...
표준 Streamlit 명령으로 실행할 때는 다음 진입 파일을 사용합니다.

```powershell
streamlit run streamlit_app.py
```

브라우저의 루트 주소(`/`)는 분석 화면인 `/main`으로 자동 이동합니다.

### Streamlit Community Cloud 배포 설정

GitHub 공개 저장소를 Streamlit Community Cloud에 연결할 때 다음과 같이 설정합니다.
...
| Python version | `3.12` |
| Secrets | 비워두기 — 이용자가 화면에서 자신의 YouTube API 키 입력 |

배포된 앱의 루트 주소는 자동으로 `/main`으로 전환되며, 분석 화면은
`https://앱주소.streamlit.app/main`에서 직접 열 수 있습니다.
